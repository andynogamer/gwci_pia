...
- Translate socket messages into EventBus payloads that already exist in CONTRACTS.md (including `ROOM_READY` → bus for Logic pad assignment, `MATCH_END` → bus so Logic can `GAME_OVER`).
